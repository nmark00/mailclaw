...
import { getEmailBodyByLookup, openEmailByLookup, createDraft, deleteEmailByLookup, replyToEmail, listAccounts, scheduleDraft } from './mail-actions.js';
    ...
    .action(async (opts, command) => {
        ...
        try {
            ...
            if (globalOpts.json) {
                ...
                console.log(chalk.green(`Draft saved: "${result.subject}"`));
            }
        } catch (error) {
...
program.command('delete <id>')
    ...
    .action(async (id, options, command) => {
        ...
        try {
            ...
            if (!lookup) throw new Error('Message not found');
            await deleteEmailByLookup(lookup);
            if (opts.json) {
                console.log(JSON.stringify({ ok: true, id }));
            } else {
                console.log(chalk.green(`Deleted message ${id}`));
            }
        } catch (error) {
            handleCommandError(error, opts);
        }
...
program.command('reply <id>')
    ...
    .action(async (id, opts, command) => {
        ...
        try {
            ...
            if (!lookup) throw new Error('Message not found');
            const result = await replyToEmail(lookup, opts.body, !!opts.all);
            if (globalOpts.json) {
                console.log(JSON.stringify({ ok: true, subject: result.subject }));
            } else {
                console.log(chalk.green(`Reply draft saved: "${result.subject}"`));
            }
        } catch (error) {
            handleCommandError(error, globalOpts);
        }
    ...
    .action(async (opts, command) => {
        ...
        try {
            ...
            if (globalOpts.json) {
                ...
                console.log(chalk.green(`Draft saved and scheduled to send at ${result.sendAt.toLocaleString()}`));
                console.log(chalk.gray(`launchd label: ${result.label}`));
            }
        } catch (error) {
            handleCommandError(error, globalOpts);
        }
